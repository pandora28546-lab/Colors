...
const LEARN = ['red','blue'];  // этап 1 → красный, этап 2 → синий
/* запрещённые (визуально похожие) соседние пары */
const FORBIDDEN = [['lightblue','blue'],['white','gray'],['gray','black'],['brown','orange']];

const KLYAKSA = { red: BASE+'Red.JPG', blue: BASE+'Blue.JPG' };

/* раскраски (этап 3, в.3–4): половинки IMG_16xx.png, полные 16xx.png */
const PAINT = { red: ['1625','1626'], blue: ['1627','1628'] };

/* банки + жуки (этап 3, в.5) */
const JAR = { red: BASE+'IMG_1587.png', blue: BASE+'IMG_1588.png' };
const BUG = { red: BASE+'1587.png',     blue: BASE+'1588.png' };  // клубника / голубика
...
function colorAudio(n, color){ return BASE + n + color + '.m4a'; }  // 1red.m4a / 1blue.m4a
...
const ESS_IMGS = [KLYAKSA.red, KLYAKSA.blue, STAR_URL, JAR.red, JAR.blue, BUG.red, BUG.blue];
COLORS.red.forEach(n => ESS_IMGS.push(objImg(n)));
COLORS.blue.forEach(n => ESS_IMGS.push(objImg(n)));
['red','blue'].forEach(c => PAINT[c].forEach(n => ESS_IMGS.push(paintHalf(n), paintFull(n))));
const essImgs = [...new Set(ESS_IMGS)];

function essAudioUrls(){
  const s = new Set();
  ['red','blue'].forEach(c => { [1,2,3,4,5].forEach(n => s.add(colorAudio(n,c))); });
  s.add(CONNECT_URL);
...
function backgroundLoad(){
  Object.keys(COLORS).forEach(c => { if(c!=='red' && c!=='blue') COLORS[c].forEach(n => { const im=new Image(); im.src=objImg(n); keepImgs.push(im); }); });
...
function pickDistractorColor(target){
  const otherLearn = target==='red' ? 'blue' : 'red';
...
function buildStage3Plan(){
  const showOrder  = shuffle(['red','blue']);
  const paintOrder = shuffle(['red','blue']);
  const ys = shuffle(PAINT.red);
  const gs = shuffle(PAINT.blue);
  return [
...
function beginStage3(){
  currentStage=2; curColor='red'; isRelearn=false;
...
function stage3Error(which){
  ...
  abaStep=0;
  if(which==='red') relearnQueue=[0];
  else if(which==='blue') relearnQueue=[1];
...
function showColorShowQuestion(step){
  ...
  setCardImg(1-correctPos, KLYAKSA[step.color==='red'?'blue':'red']);
  unlockCards();
  updateProgressStage3();
...
function showPaintQuestion(step){
  ...
  showPane('cards'); resetCards();
  correctPos = Math.random()<.5 ? 0 : 1;
  const askNum   = step.color==='red' ? step.y : step.g;
  const otherNum = step.color==='red' ? step.g : step.y;
...
function onPaintAnswer(pos){
  ...
  answered=true; lockCards();
  if(pos===correctPos){
    const askNum = curColor==='red' ? curPaint.y : curPaint.g;
...
function buildBanks(){
  ...
  const order = Math.random()<.5 ? ['red','blue'] : ['blue','red'];
  ...
  const bugColors = shuffle(['red','red','red','blue','blue','blue']);
  const bugW = mob?22:9;
